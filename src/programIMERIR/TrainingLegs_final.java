...
public class TrainingLegs_final extends RoboticsAPIApplication {
	...
	public void run() {
		...
		answer2 = getApplicationUI().displayModalDialog(
				ApplicationDialogType.QUESTION,
				"Quel jambe voulez vous entrainer ?",
				"Droite", "Gauche", "STOP");
		...
		while (answer2 != 2){			
			...
			case halima:
				...
				while (answer != 1) {
					...
					answer = getApplicationUI().displayModalDialog(
							ApplicationDialogType.QUESTION,
							"Voulez vous refaire un cycle sur cette jambe?", "Oui", "Non");
				}
				answer = -1;
			...
			case thomas:
				...
				leg_thomas.getFrame("/PNP_enfant_thomas").attachTo(legLift.getFrame("TCP"));
				...
				break;
			case mathis:
				leg_mathis.getFrame("/PNP_enfant_mathis").attachTo(legLift.getFrame("TCP"));
				while (answer != 1) {
