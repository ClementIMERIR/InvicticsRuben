...
public class HoldCompliance extends RoboticsAPIApplication {
	...
	private ForceCondition AdjustZPos, AdjustZNeg;
	...
	private ICallbackAction AdjustZ = new ICallbackAction() {
		
		@Override
		public void onTriggerFired(IFiredTriggerInfo triggerInformation) {
			penCollisionForce = robot.getExternalForceTorque(robot.getFrame("Pen/PenTCP/PenAlignWorld"));
			Vector penForces = data.getForce();
			getLogger().info("PenForces : " + penForces.getX() + " , " + penForces.getY() + " , " + penForces.getZ());
			if(penForces.getZ() > 0){
	...
	public void initialize() {
		...
		//Condition de force sur l'axe 7 du bras
		AdjustZPos = ForceCondition.createNormalForceCondition(penWorldAlign, CoordinateAxis.Z, 2);
		//AdjustZNeg = ForceCondition.createNormalForceCondition(penWorldAlign, CoordinateAxis.Z, -2);
		...
		penCollision = ForceCondition.createSpatialForceCondition(penWorldAlign, 7);
	...
	public void drawSquare(double startingPointX, double startingPointY, double dimension){
		...
		//move relatif
		penWorldAlign.move(linRel(squareSize,0,0).triggerWhen(AdjustZPos, AdjustZ));
		penWorldAlign.move(linRel(0,squareSize,0).triggerWhen(AdjustZPos, AdjustZ));
		penWorldAlign.move(linRel(-squareSize,0,0).triggerWhen(AdjustZPos, AdjustZ));
		penWorldAlign.move(linRel(0,-squareSize,0).triggerWhen(AdjustZPos, AdjustZ));
