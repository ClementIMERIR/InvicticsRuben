...
public class HoldCompliance extends RoboticsAPIApplication {
	...
	public void run() {
		...
		robot.move(ptp(getApplicationData().getFrame("/WorkingTable/P5")));
		Frame currentFrame = robot.getCurrentCartesianPosition(robot.getFlange());
		PositionInformation posInfo = robot.getPositionInformation(robot.getFlange());
		getLogger().info("transDiff = " + posInfo.getTranslationOffset().getX() + " , "
				+ posInfo.getTranslationOffset().getY() + " , "
				+ posInfo.getTranslationOffset().getZ());
		getLogger().info("rotDiff = " + posInfo.getRotationOffset().getAlphaRad() + " , "
				+ posInfo.getRotationOffset().getBetaRad() + " , "
				+ posInfo.getRotationOffset().getGammaRad());
		robot.move(linRel(currentFrame.getX(), currentFrame.getY(), currentFrame.getZ(), Math.toRadians(-180), Math.toRadians(0), Math.toRadians(180)));
