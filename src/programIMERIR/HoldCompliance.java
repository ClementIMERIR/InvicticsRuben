...
public class HoldCompliance extends RoboticsAPIApplication {
	...
	private IUserKeyListener startDrawingListener = new IUserKeyListener() {
		...
		public void onKeyEvent(IUserKey key, UserKeyEvent event) {
			if(event == UserKeyEvent.KeyDown){
				...
				//draw();
				robot.move(linRel(penTCP.getX(), penTCP.getY(), 0));
			}
		}
	};
	...
	public void initialize() {
		...
		buttonsBar = getApplicationUI().createUserKeyBar("Mouvement");
		...
		penCollision = ForceCondition.createNormalForceCondition(robot.getFrame("/Pen/PenTCP"), CoordinateAxis.Z, 2);
		...
		penTCP = pen.getFrame("/Pen/PenTCP");
	}
