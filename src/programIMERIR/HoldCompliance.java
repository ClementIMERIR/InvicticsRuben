...
public class HoldCompliance extends RoboticsAPIApplication {
	...
	public void freeMovementRobot(){
		...
		do{
			...
			robot.move(positionHold(freeMode,500, TimeUnit.MILLISECONDS));
		}while(sumForces >= 5);
		...
		//robot.move(linRel(currFrameState.getX(), currFrameState.getY(), currFrameState.getZ(),Math.toRadians(-180), Math.toRadians(0), Math.toRadians(180)));
	}
}