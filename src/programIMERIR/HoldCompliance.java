...
public class HoldCompliance extends RoboticsAPIApplication {
	...
	IUserKey startSquareDrawingCompliant;
	IUserKey startCircleDrawingCompliant;
	IUserKey startHeartDrawingCompliant;
	 ...
	 */
	private IUserKeyListener startCircleDrawingCompliantListener = new IUserKeyListener() {
		@Override
...
//				//grabForceObserver.disable();
	...
	private IUserKeyListener buttonPushedListener = new IUserKeyListener() {
		
		@Override
		public void onKeyEvent(IUserKey key, UserKeyEvent event) {
			// TODO Auto-generated method stub
			int button = key.getSlot();
			squareSize = getApplicationData().getProcessData("squareSize").getValue();
			getLogger().info("Button startDrawing triggered");
			Frame currentFrame = robot.getCurrentCartesianPosition(penWorldAlign);
			getLogger().info("penInfos = " + currentFrame.getX() + " , " + currentFrame.getY() + " , "  + currentFrame.getZ());
//			//grabForceObserver.disable();
		...
		public void onRisingEdge(ConditionObserver conditionObserver, Date time,
			...
			switch(drawingMethode){
				...
				case 2:
					...
					break;
				case 3:
					drawHeartCompliance(currentFrame.getX(), currentFrame.getY(), 50);
					break;
			}
	...
	public void initialize() {
		...
		startSquareDrawingCompliant = buttonsBar.addUserKey(0, buttonPushedListener, true);
		startSquareDrawingCompliant.setText(UserKeyAlignment.MiddleLeft, "Start Drawing Compliant");
		
		//Ajout d'un bouton pour lancer le dessin avec recalcul des z manuel
		startDrawingZCalc = buttonsBar.addUserKey(1, buttonPushedListener, true);
		startDrawingZCalc.setText(UserKeyAlignment.MiddleLeft, "Start Drawing Z Calc");
		
		startCircleDrawingCompliant = buttonsBar.addUserKey(2, buttonPushedListener,true);
		startCircleDrawingCompliant.setText(UserKeyAlignment.MiddleLeft, "Start Drawing Circle");
		
		startHeartDrawingCompliant = buttonsBar.addUserKey(3, buttonPushedListener,true);
		startHeartDrawingCompliant.setText(UserKeyAlignment.MiddleLeft, "Start Drawing Heart");
		buttonsBar.publish();
...
//		//avec le move de l'API
	...
	public void drawCircleCompliance(double startingPointX, double startingPointY, double rayon){
		getLogger().info("Début du dessin du rond");
		double altitude = 530;
		Frame p0 = new Frame(startingPointX,startingPointY,altitude);
...
//		//avec le move de l'API
	...
	public void drawTriangleCompliance(double startingPointX, double startingPointY, double size){
		getLogger().info("Début du dessin du triangle");
		double altitude = 530;
		Frame p0 = new Frame(startingPointX, startingPointY, altitude);
		Frame p1 = new Frame(p0.getX(), p0.getY()-size, altitude);
		Frame p2 = new Frame(p0.getX()+(Math.pow(size, 2)-Math.pow(size/2, 2)), p0.getY()+size/2, altitude);
		getLogger().info("p0:("+p0.getX()+","+p0.getY()+")\n"
				+"p1:("+p1.getX()+","+p1.getY()+")\n"
				+"p2:("+p2.getX()+","+p2.getY());
		...
		penWorldAlign.move(lin(p2).setMode(drawMode).setCartVelocity(50).breakWhen(grabForce));
		penWorldAlign.move(lin(p0).setMode(drawMode).setCartVelocity(50).breakWhen(grabForce));
	}
	
	public void drawHeartCompliance(double startingPointX, double startingPointY, double size){
		...
		//avec le move de l'API
		penWorldAlign.move(lin(p1).setMode(drawMode).setCartVelocity(50).breakWhen(grabForce));
		penWorldAlign.move(circ(p2, p3).setMode(drawMode).setCartVelocity(50).breakWhen(grabForce));
		penWorldAlign.move(circ(p4, p5).setMode(drawMode).setCartVelocity(50).breakWhen(grabForce));
		penWorldAlign.move(lin(p0).setMode(drawMode).setCartVelocity(50).breakWhen(grabForce));
	}
}