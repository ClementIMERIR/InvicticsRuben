...
//		test2.move(lin(test));
		...
		for(double i = framePoints.get(0).getX(); i < framePoints.get(3).getX(); i += largeurOutil) {
			...
			pliers.getFrame("Sander").move(linRel(0.0, framePoints.get(1).getY(), 0.0).setJointVelocityRel(0.5));
			pliers.getFrame("Sander").move(linRel(0.0, 0.0, 60.0).setJointVelocityRel(0.5));
			pliers.getFrame("Sander").move(linRel(0.0, -framePoints.get(1).getY(), 0.0).setJointVelocityRel(0.5));
			pliers.getFrame("Sander").move(linRel(0.0, 0.0, -50.0).setJointVelocityRel(0.5));
			pliers.getFrame("Sander").move(linRel(i, 0.0, 0.0).setJointVelocityRel(0.5));
		}
		/*-------------------------------------------------------------------------------------------------------------------*/
	...
	/**
	 * Sorts the list so that the first and last point have the tiniest and greatest X coordinate value
	 * @param framePoints
	 */
	private void sort(ArrayList<ObjectFrame> framePoints) {
		ObjectFrame tiniestFrame = null;
		ArrayList<ObjectFrame> sortedList = new ArrayList<ObjectFrame>();
		
		while (framePoints.size() > 0) {
			for(ObjectFrame point : framePoints){
				tiniestFrame = point.getX() < tiniestFrame.getX() ? point : tiniestFrame;
				framePoints.remove(point);
			}
			sortedList.add(tiniestFrame);
		}
		
		framePoints = sortedList;
	}
}