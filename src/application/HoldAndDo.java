...
//		mode.setStiffness(10, 10, 10, 10, 10, 10, 1);
	...
	private void polish(){
		...
		pliers.getFrame("/Sander").move( linRel(framePoints.get(0).getX(), framePoints.get(0).getY(), framePoints.get(0).getZ()) );
		
		for(double i = framePoints.get(0).getX(); i < framePoints.get(3).getX(); i += largeurOutil) {
			pliers.getFrame("/Sander").move(linRel(i, framePoints.get(0).getY(), 0.0).setJointVelocityRel(1.0));//retour depart
			pliers.getFrame("/Sander").move(linRel(i, framePoints.get(2).getY(), 0.0).setJointVelocityRel(0.25));//ponçage
		}
